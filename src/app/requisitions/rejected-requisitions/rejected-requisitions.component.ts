...
@Component({
  ...
  styleUrls: ['./rejected-requisitions.component.scss'],
})
export class RejectedRequisitionsComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  loadRejectedRequisitions(): void {
    ...
    this.warehouseService
      ...
      .subscribe({
        next: (data) => {
          console.log('✅ Rejected requisitions loaded:', data);
        ...
        error: (err) => {
          console.error('❌ Error loading rejected requisitions:', err);
          this.isLoading = false;
          Swal.fire({
            icon: 'error',
            title: 'Error',
            text: 'Failed to load rejected requisitions',
          });
        },
      });
  }
